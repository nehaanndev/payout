...
import { calculateBalancesForSummary } from '@/lib/financeUtils';
...
interface SummaryProps {
  ...
  export default function Summary({ groups, expensesByGroup, onSettleClick }: SummaryProps) {
    if (groups.length === 0) {
      return (
        <Card className="mt-6">
    ...
    return (
      <>
        {groups.map(group => {
          ...
          const balances = calculateBalancesForSummary(group.members, expenses);
            ...
            <Card key={group.id} className="mt-6">
              ...
              <CardContent className="space-y-3">
                <p className="text-sm text-gray-600">
                  Total spent: <b>${expenses.reduce((s, e) => s + e.amount, 0).toFixed(2)}</b>
                </p>
                  ...
                  <>
                    <ul className="text-sm">
                      {Object.entries(balances).map(([id, bal]) => {
                        ...
                        const name = group.members.find(m => m.id === id)?.firstName ?? id;
                        return (
                          <li key={id} className={bal < 0 ? "text-red-600" : "text-green-600"}>
                            {name} {bal < 0 ? "owes" : "is owed"} ${Math.abs(bal).toFixed(2)}
                          </li>
                        );
                      })}
                    </ul>
