...
export function splitByWeights(amountMinor: number, weights: Record<string, number>) {
  ...
  for (let i = 0; i < remainder; i++) {
    const id = raw[i].id;
    const slot = floored.find(f => f.id === id)!;
    slot.val += 1;
  }

  const out: Record<string, number> = {};
  for (const f of floored) out[f.id] = f.val;
  return out; // exact sum match
}
